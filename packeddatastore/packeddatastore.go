...
func (pdsi *packedDataStoreImpl) AddBlock(bl elements.ExtendedBlock) {
    ...
    for i:=0; i < bl.Len(); i++ {
        ...
        switch gt {
            case geometry.Point: pdsi.point[k]=append(pdsi.point[k],or)
            case geometry.Linestring: pdsi.line[k]=append(pdsi.line[k],or)
            case geometry.Polygon, geometry.Multi: pdsi.polygon[k]=append(pdsi.polygon[k],or)
        }
...
func (pdsi *packedDataStoreImpl) Filter(bb quadtree.Bbox, gt geometry.GeometryType, cols []string) sqlselect.Result {
    ...
    switch gt {
        case geometry.Point: objs = pdsi.point
        case geometry.Linestring: objs = pdsi.line
        case geometry.Polygon: objs = pdsi.polygon
    }
    if objs==nil {
        return nil
    }
    ...
    for k,v := range objs {
        ...
        for _,o := range v {
            ...
            if !ok {
                fmt.Println("?? not a geometry", elements.PackedElement(gd))
                continue
            }
            ...
            for i,c := range cols {
                switch c {
                    ...
                    case "way": r[i] = sqlselect.GeomValue(pg.GeometryData())
                    case "z_order": r[i] = sqlselect.IntValue(zo)
                    case "way_area": r[i] = sqlselect.FloatValue(ar)
                    case "quadtree": r[i] = sqlselect.IntValue(int64(pg.Quadtree()))
    ...
    for _,pp := range [][]objRow{pt,ln,py} {    
        if pp!=nil {
            for _,o := range pp {
                ...
                ele.(interface{SetTags(elements.Tags) }).SetTags(elements.MakeTags(kk,vv))
