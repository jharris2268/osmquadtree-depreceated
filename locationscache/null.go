...
func (fflc *nullLocationsCache) FindTiles(inc <-chan int64) (Locs, TilePairSet) {
	...
	for fl, idx := range fflc.idx {
		...
		log.Println("scan", fflc.prfx+idx.Filename)
			...
			go func(i int) {
				for bl := range tiles[i] {
					...
					for j := 0; j < bl.Len(); j++ {
						...
						if k == 2524940079 {
							log.Println(fl, tl, e, e.ChangeType())
						}
		...
		wg.Wait()
		log.Println("found", len(ts[0])+len(ts[1])+len(ts[2])+len(ts[3]), "elements")
		for _, t := range ts {
			for k, v := range t {
				if v == -1 {
					ll[k] = TilePair{-1, -1}
