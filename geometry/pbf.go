...
func extractGeometryData(indata []byte) (
	GeometryType, elements.ElementType, [][][]Coord, int64, float64, error) {
	...
	for msg.Tag > 0 {
		...
		case 15:
			...
			objs = append(objs, py)
        case 17:
            ot = elements.ElementType(msg.Value)
		}
		if err != nil {
			return 0, elements.None, nil, 0, 0, err
		}
		pos, msg = utils.ReadPbfTag(indata, pos)
	}

	return gt, ot, objs, zo, ar, nil
...
func extractGeometry(gp elements.PackedGeometry) (Geometry, error) {
	gt, ot, objs, zorder, area, err := extractGeometryData(gp.GeometryData())
	if err != nil {
		return nil, err
	}
	...
	switch gt {
	case Point:
		return makePointGeometry(gp, ot, gp.Tags(), objs[0][0][0]), nil
	case Linestring:
		return makeLinestringGeometry(gp, ot, gp.Tags(), objs[0][0], zorder), nil
	case Polygon:
		return makePolygonGeometry(gp, ot, gp.Tags(), objs[0], zorder, area), nil
	case MultiPoint, MultiLinestring, MultiPolygon, Multi:
		return makeMultiGeometry(gp, ot, gp.Tags(), objs, zorder, area), nil
	}
...
func packPointData(ot elements.ElementType, c Coord) []byte {

	msgs := make(utils.PbfMsgSlice, 3)
	msgs[0] = utils.PbfMsg{10, nil, 1}
	msgs[1] = utils.PbfMsg{13, packCoord(c), 0}

    msgs[2] = utils.PbfMsg{17, nil, uint64(ot)}
...
func packLinestringData(ot elements.ElementType, cc []Coord, zo int64, bb *quadtree.Bbox) []byte {
	msgs := make(utils.PbfMsgSlice, 0, 5)
	msgs = append(msgs, utils.PbfMsg{10, nil, 2})
	msgs = append(msgs, utils.PbfMsg{11, nil, utils.Zigzag(zo)})
	msgs = append(msgs, utils.PbfMsg{14, packRing(cc), 0})
	if bb != nil {
		msgs = append(msgs, utils.PbfMsg{16, packBbox(*bb), 0})
	}
    msgs = append(msgs, utils.PbfMsg{17, nil, uint64(ot)})
	return msgs.Pack()
}
...
func packPolygonData(ot elements.ElementType, cc [][]Coord, zo int64, ar float64, bb *quadtree.Bbox) []byte {
	msgs := make(utils.PbfMsgSlice, 0, 6)
	msgs = append(msgs, utils.PbfMsg{10, nil, 3})
	msgs = append(msgs, utils.PbfMsg{11, nil, utils.Zigzag(zo)})
	msgs = append(msgs, utils.PbfMsg{12, nil, math.Float64bits(ar)})
	msgs = append(msgs, utils.PbfMsg{15, packPolygon(cc), 0})
	if bb != nil {
		msgs = append(msgs, utils.PbfMsg{16, packBbox(*bb), 0})
	}
    msgs = append(msgs, utils.PbfMsg{17, nil, uint64(ot)})
...
func packMultiGeometryData(ot elements.ElementType, cc [][][]Coord, zo int64, ar float64, bb *quadtree.Bbox) []byte {
	msgs := make(utils.PbfMsgSlice, 0,5+len(cc))
	msgs = append(msgs, utils.PbfMsg{10, nil, 7})
	msgs = append(msgs, utils.PbfMsg{11, nil, utils.Zigzag(zo)})
	msgs = append(msgs, utils.PbfMsg{12, nil, math.Float64bits(ar)})
	for _, c := range cc {
		msgs = append(msgs, utils.PbfMsg{15, packPolygon(c), 0})
	}
	if bb != nil {
		msgs = append(msgs, utils.PbfMsg{16, packBbox(*bb), 0})
	}
