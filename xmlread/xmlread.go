...
func ReadOsmXmlFile(filename string) (<-chan elements.Element, error) {
    ...
    addf := func(i int, b elements.ExtendedBlock) error {
        for j:=0; j < b.Len(); j++ {
            elems <- b.Element(j)
        }
        return nil
    ...
    go func() {
        ReadXmlBlocks(filename,addf)
        close(elems)
    }()
    return elems,nil
}
    
func MakeReadXmlBlocks(filename string) func(func(int, elements.ExtendedBlock) error) error {
    return func(addf func(int, elements.ExtendedBlock) error) error {
        return ReadXmlBlocks(filename,addf)
    }
}

func ReadXmlBlocks(filename string, addFunc func(int, elements.ExtendedBlock) error) error {
    ...
    err = func() error {
        for {
            ...
            case xml.EndElement:
                ...
                if len(ts)==cap(ts) {
                    addFunc(0, elements.MakeExtendedBlock(ii, ts, -1,0,0,nil))
                    ii++
                    ts=make(elements.ByElementId, 0, 8000)
                }
            }
