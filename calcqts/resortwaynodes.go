...
func readWayNodes(infn string, nc int, tfs string, sp uint) (blocksort.AllocBlockStore, int, elements.Ref,elements.Block, error) {
    ...
    if err!=nil { return nil,0,0,nil,err}
    ...
    fmt.Println("have",nw,"ways","max=",mx)
...
func expandWayBoxes(nodeWays nwbs, mw elements.Ref, Mw elements.Ref, storeType int) wayBbox {
    ...
    fmt.Println(mw,"to",Mw,"=>",(Mw-mw)/tileSz+1,"tiles; reqgc?",reqgc)
	...
	fmt.Printf("\rexpandWayBoxes: %-12d // %-12d: %-8.1fs  %-5d tiles %s %-50s\n", numNode, numWN, time.Since(st).Seconds(),ans.NumTiles(),utils.MemstatsStr(), "")
	fmt.Printf("%-10d wayboxes [between %-10d and %-10d]\n", ans.Len(), mw, Mw)
    fmt.Printf("called debug.FreeOSMemory() %d times [%8.1fs]",z,tz)
    if reqgc {
        fmt.Printf("; runtime.GC() %d times [%8.1fs]\n",ngc,gct)
    } else {
        fmt.Printf("\n")
    }
	return ans
...
func findNodeQts(
		...
		for j:=0; j < mm.Len(); j++ {
            switch mm.MemberType(j) {
                ...
                case elements.Way:
                    ...
                    if q<0 {
                        if (mw < 10) {
                            fmt.Printf("%5d MISSING WAY QT: relation %d, way %d\n",mw,e.Id(),mm.Ref(j))
                        }
                        mw++
                    }
                    rls.Expand(e.Id(), q)
    ...
    fmt.Println("missing",mw,"way qts")
    ...
    fmt.Printf("have %d missing node qts:\n", len(ndrel))
    zz:=0
    for k,v:=range ndrel {
        if zz<10 {
            fmt.Printf("% 2d node %d, rels %s\n",zz,k,v)
        }
        zz++
    }
...
func writeWayQts(wayQts quadtreeStore,
    ...
    fmt.Printf("wrote %d way blocks in %8.1fs\n", kk, time.Since(st).Seconds())
...
func CalcObjectQts(infn string, storeType int, tfs string) (<-chan elements.ExtendedBlock, error) {
	...
	go func() {
        ...
        fmt.Printf("read way nodes: %6.1fs\n", t1.Seconds())
        fmt.Printf("calc way qts:   %6.1fs\n", t2.Seconds())
        fmt.Printf("calc node qts:  %6.1fs\n", t3.Seconds())
        fmt.Printf("write way qts:  %6.1fs\n", t4.Seconds())
        fmt.Printf("calc rel qts:   %6.1fs\n", t5.Seconds())
        fmt.Printf("TOTAL:          %6.1fs\n", tt.Seconds())
